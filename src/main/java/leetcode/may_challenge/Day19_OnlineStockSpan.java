...
public class Day19_OnlineStockSpan {
	Stack<int[]> price_span;

	public Day19_OnlineStockSpan() {
		price_span = new Stack<>();
	}

	public int next(int price) {
		int span = 1;
		while (!price_span.isEmpty() && price_span.peek()[0] <= price) {
			span += price_span.pop()[1];
		}
		price_span.push(new int[] { price, span });
		return span;
	}
}
