...
public class Day11_FloodFill {
	...
	public int[][] floodFill(int[][] image, int sr, int sc, int newColor, int startPixelColor) {
		if (sr >= 0 && sr < image.length && sc >= 0 && sc < image[0].length && image[sr][sc] == startPixelColor) {
			image[sr][sc] = newColor;
			floodFill(image, sr - 1, sc, newColor, startPixelColor);
			floodFill(image, sr + 1, sc, newColor, startPixelColor);
			floodFill(image, sr, sc + 1, newColor, startPixelColor);
			floodFill(image, sr, sc - 1, newColor, startPixelColor);
		}
		return image;
	}
